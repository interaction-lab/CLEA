"""
Auditory (audio spectrogram) representation models.

All models accept (B, C, H, W) tensors of log-mel spectrogram images.
The architecture mirrors the visual models but uses audio-specific decoder padding.
"""
...
class RawAudioEncoder(nn.Module):
    """Convolutional encoder mapping a spectrogram to a fixed-size embedding."""
...
class RawAudioDecoder(nn.Module):
    """Transposed-conv decoder mapping an embedding back to a spectrogram."""
    ...
    def __init__(self, intermediate_size, reshape_size, hidden_dim=256, latent_dim=32,
        ...
        self.conv_transpose_layers = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.BatchNorm2d(32),
            nn.ConvTranspose2d(32, 16, kernel_size=(8, 8), stride=(4, 4), output_padding=(1, 0)),
            nn.LeakyReLU(),
            nn.ConvTranspose2d(16, 3, kernel_size=(16, 16), stride=(4, 4), output_padding=(0, 2)),
            nn.Sigmoid(),
        )
...
class RawAudioAE(nn.Module):
    """Autoencoder over raw audio spectrograms."""

    def __init__(self, input_dim, hidden_dim=256, latent_dim=32, device='cuda:0'):
        super().__init__()
        self.device = device
        self.encoder = RawAudioEncoder(input_dim, hidden_dim, latent_dim, device)
        self.decoder = RawAudioDecoder(
...
class RawAudioVAE(nn.Module):
    """
    Variational autoencoder over raw audio spectrograms.
