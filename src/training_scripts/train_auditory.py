"""
Train auditory (audio spectrogram) representation models.
...
from clea.models.auditory import RawAudioEncoder, RawAudioAE, RawAudioVAE
...
def get_dataloader(batch_size: int, signal: str):
    ...
    df = df.query(f'type == "auditory" & signal == "{signal}"')
    dataset = RawChoiceDataset(df, kind='auditory', transform=torch.Tensor, data_dir=DATA_DIR)
...
def get_model_and_loss(model_type: str, input_dim, hidden_dim: int, latent_dim: int,
    ...
    if model_type == 'contrastive':
        return RawAudioEncoder(input_dim, hidden_dim, latent_dim, device=device), \
        ...
        model = RawAudioVAE(input_dim, hidden_dim, latent_dim, device=device, beta=beta)
...
def train(model_type: str, device: str = 'cpu', batch_size: int = 128,
          ...
          margin: float = 0.1, beta: float = 10.0, hidden_dim: int = 512,
          skip_existing: bool = False):
    """
    Train an auditory representation model for all four exploratory signals.
    The auditory encoder architecture is a CNN that operates on spectrogram inputs.
    ...
    for signal in SIGNALS:
        save_path = os.path.join(
            MODEL_SAVE_DIR, f'auditory&independent&raw&{model_type}&{signal}&{embedding_dim}.pth'
        )
        if skip_existing and os.path.exists(save_path):
            print(f'Skipping auditory | signal={signal} | type={model_type} | dim={embedding_dim} (already exists)')
            continue

        print(f'Training auditory | signal={signal} | type={model_type} | dim={embedding_dim}')
...
if __name__ == '__main__':
    ...
    # AUDITORY override dict there instead of this file.
    ...
    for embedding_dim in cfg.EMBEDDING_DIMS:
        for model_type in cfg.MODEL_TYPES:
            train(model_type, device, cfg.BATCH_SIZE, embedding_dim, cfg.LR, cfg.NUM_EPOCHS,
                  cfg.AUDITORY['margin'], cfg.AUDITORY['beta'], cfg.AUDITORY['hidden_dim'],
